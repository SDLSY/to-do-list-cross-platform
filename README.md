...
## 🤖 5. Human-in-the-Loop AI Agent 自动化协作体系

项目深度内置了专为 AI Agent 设计的结构化任务调度扩展机制，实现了 **“用户负责提需求与验收成果，AI Agent 负责自动执行”** 的人机协同新范式。

### 1. 卡片规格扩展与严格填参
每张任务卡片均可独立开启 **【🤖 委派给 AI Agent 全自动执行】** 开关：
- 📂 **工作目录 (WorkDir)**：严格限定 Agent 在指定的工程路径中工作，防止越权；
- 🧠 **执行模型 (Model)**：支持针对任务难度选择模型（如 Claude 3.5 Sonnet / DeepSeek V3 / GPT-4o）；
- ⏱️ **最大执行步数 (Max Steps)**：设定步数上限，杜绝无限死循环消耗 Token；
- 📜 **实时现场日志 (agent_log)**：Agent 运行过程中的每一步推理、终端命令与文件改动均实时写回卡片。

### 2. 调度守护进程 (Agent Dispatcher)
项目在 `agent/agent_dispatcher.py` 中提供了开箱即用的常驻监听调度器：
1. 通过 PocketBase SSE 原生长连接毫秒级捕获带有 `is_agent: true` 的待办任务；
2. 自动认领并更新状态为 `in_progress`；
3. 严格切换至卡片指定的 `work_dir` 执行任务；
4. 遇到异常或命令错误时，**自动保留完整现场堆栈并挂起**，杜绝黑盒死机；
5. 用户随时点击卡片上的 **`📜 日志`** 按钮，一览执行明细，并一键完成 **“✓ 验收通过”** 或 **“↺ 驳回重做”**。
